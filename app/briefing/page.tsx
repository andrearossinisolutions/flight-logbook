...
// Fallback skeleton loader per il caricamento asincrono dei METAR/TAF
function StationDetailsSkeleton() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24, width: "100%", marginTop: 24 }}>
      {/* Sintesi della rotta skeleton */}
      <div className="card skeleton-card" style={{ padding: 18 }}>
        <div className="skeleton-title" style={{ width: 180, height: 20, marginBottom: 12 }}></div>
        <div style={{ display: "flex", gap: 12 }}>
          <div className="skeleton-pill" style={{ width: 80, height: 32 }}></div>
          <div className="skeleton-pill" style={{ width: 80, height: 32 }}></div>
        </div>
      </div>

      {/* Partenza/Destinazione details skeleton */}
      <div className="grid grid-2 stack-mobile" style={{ gap: 24 }}>
        <div className="card skeleton-card" style={{ padding: "20px 24px" }}>
          <div className="skeleton-title" style={{ width: 140, height: 22, marginBottom: 16 }}></div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div className="skeleton-line" style={{ height: 16 }}></div>
            <div className="skeleton-line" style={{ height: 16 }}></div>
            <div className="skeleton-line" style={{ height: 16 }}></div>
            <div className="skeleton-line" style={{ height: 40, marginTop: 8 }}></div>
          </div>
        </div>
        <div className="card skeleton-card" style={{ padding: "20px 24px" }}>
          <div className="skeleton-title" style={{ width: 140, height: 22, marginBottom: 16 }}></div>
      ...
      {/* Stazione details skeleton */}
      <div className="card skeleton-card" style={{ padding: 24 }}>
        <div className="between" style={{ marginBottom: 20 }}>
          <div>
            <div className="skeleton-text" style={{ width: 100, height: 12, marginBottom: 6 }}></div>
            <div className="skeleton-title" style={{ width: 220, height: 28, marginBottom: 8 }}></div>
            <div className="skeleton-text" style={{ width: 300, height: 14 }}></div>
          </div>
          <div className="skeleton-badge" style={{ width: 80, height: 40 }}></div>
        </div>
        
        <div className="weather-grid" style={{ marginBottom: 28 }}>
          <div className="weather-card skeleton-card" style={{ height: 110 }}></div>
          <div className="weather-card skeleton-card" style={{ height: 110 }}></div>
          <div className="weather-card skeleton-card" style={{ height: 110 }}></div>
          <div className="weather-card skeleton-card" style={{ height: 110 }}></div>
        </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .skeleton-title, .skeleton-pill, .skeleton-line, .skeleton-text, .skeleton-badge {
...
// Componente per il caricamento asincrono dei dati meteo delle stazioni
async function BriefingStationDetails({
  icaos,
  targetIcao,
  defaultBase,
}: {
...
export default async function BriefingPage({
  ...
  const mapPoints: { lat: number; lon: number }[] = [];
  if (icaos.length > 0) {
    for (const icaoCode of icaos) {
      const apt = ITALIAN_AIRPORTS[icaoCode];
      if (apt?.lat !== undefined && apt?.lon !== undefined) {
        mapPoints.push({ lat: apt.lat, lon: apt.lon });
      } else {
        const coords = await getCoordinatesFromName(icaoCode);
        if (coords?.lat !== undefined && coords?.lon !== undefined) {
          mapPoints.push({ lat: coords.lat, lon: coords.lon });
        }
      }
    }
  }
  ...
  if (mapPoints.length > 0) {
    const lats = mapPoints.map(c => c.lat);
    const lons = mapPoints.map(c => c.lon);
  ...
  return (
    ...
    >
      ...
      {!showIntro && (
        <Suspense fallback={<StationDetailsSkeleton />} key={targetIcao}>
          <BriefingStationDetails 
            icaos={icaos} 
            targetIcao={targetIcao} 
